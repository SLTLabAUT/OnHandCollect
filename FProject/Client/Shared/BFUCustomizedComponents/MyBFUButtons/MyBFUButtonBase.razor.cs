...
{
    public partial class MyBFUButtonBase : ButtonParameters, IAsyncDisposable
    {
        public MyBFUButtonBase()
        {
        ...
        static List<MyBFUButtonBase> radioButtons = new();
        ...
        protected RenderFragment<MyBFUButtonBase> CustomBuildRenderTree = button => builder =>
        {
        ...
        protected async void ClickHandler(MouseEventArgs args)
        {
            ...
            if (IsRadioButton)
            {
                isChecked = true;
                await CheckedChanged.InvokeAsync(isChecked);
                foreach (MyBFUButtonBase bFUButtonBase in radioButtons)
                {
