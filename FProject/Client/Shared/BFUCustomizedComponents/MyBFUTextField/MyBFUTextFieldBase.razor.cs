...
{
    public class MyBFUTextField : MyBFUTextFieldBase<string?>
    {
    ...
    public partial class MyBFUTextFieldBase<TValue> : MyBFUTextFieldParameters, IAsyncDisposable
    {
        ...
        /// </summary
        [Parameter] public string ParsingErrorMessage { get; set; } = "The {0} field must be a number within the valid range.";
        ...
        /// </summary>
        public MyBFUTextFieldBase()
        {
            _validationStateChangedHandler = OnValidateStateChanged;
        }

        static MyBFUTextFieldBase()
        {
        ...
        protected override Task OnParametersSetAsync()
        {
            ...
            if (ValidateAllChanges() && ValidateOnLoad)
            {
                Validate(CurrentValue);
            }

            if (Label is not null && DisplayName is null)
            {
                DisplayName = Label;
            }
        ...
        private void Validate(TValue? value)
        {
            if (CascadedEditContext != null && ValueExpression != null)
            {
                ...
                {
                    ErrorMessage = string.Join("<br>", CascadedEditContext.GetValidationMessages(FieldIdentifier));
                }
                else
                {
                    ErrorMessage = "";
